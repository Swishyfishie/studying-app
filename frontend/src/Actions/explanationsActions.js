...
export const fetchExplanations = () => {
    return (dispatch) => {
        fetch("http://localhost:3001/explanations")
            .then(resp => resp.json())
...
//     console.log(obj.id)
//     fetch(`http://localhost:3001/explanations/${obj.id}`, {
//         method: "DELETE"
//     })
...
export const deleteExplanation = (obj) => {
    return (dispatch) => {
        fetch(`http://localhost:3001/explanations/${obj.id}`, {
            method: 'DELETE'
        })
            .then(res => res.json())
            .then(exp => dispatch({ type: 'DELETE_EXPLANATION', obj }))
    }
